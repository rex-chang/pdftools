...
final class InvoiceParserUnitTests: XCTestCase {
    ...
    func testLeadingAmount_plain() {
        XCTAssertEqual(InvoiceParser.leadingAmount(in: "2000.00"), "2000.00")
    }

    func testLeadingAmount_withCommas() {
        XCTAssertEqual(InvoiceParser.leadingAmount(in: "1,234,567.89"), "1234567.89")
    }

    func testLeadingAmount_leadingNoise() {
        // "abc123" has no leading amount → nil.
        XCTAssertNil(InvoiceParser.leadingAmount(in: "abc123"))
    }

    func testLeadingAmount_negative() {
        XCTAssertEqual(InvoiceParser.leadingAmount(in: "-2.94"), "-2.94")
    }

    func testLeadingAmount_integer() {
        XCTAssertEqual(InvoiceParser.leadingAmount(in: "1899"), "1899")
    }

    func testLeadingAmount_trailingNoise() {
        // Only the leading amount is captured; "元" is left out.
        XCTAssertEqual(InvoiceParser.leadingAmount(in: "218.60元"), "218.60")
    }

    // MARK: extractType

    func testExtractType_singleSegment() {
        XCTAssertEqual(InvoiceParser.extractType(in: "*餐饮服务*"), "餐饮服务")
    }
    ...
    func testExtractType_doubleSegment() {
        ...
        XCTAssertEqual(InvoiceParser.extractType(in: "*生产生活服务*餐费*"), "生产生活服务")
    }

    func testExtractType_jdPrepaidCard() {
        XCTAssertEqual(InvoiceParser.extractType(in: "*预付卡销售*京东E卡"), "预付卡销售")
    }

    func testExtractType_none() {
        XCTAssertNil(InvoiceParser.extractType(in: "没有星号包裹的类型"))
    }
    ...
    func testPickAmountPair_twoYenAmounts() {
        let cands = [
            InvoiceParser.AmountCandidate(x: 100, amount: "197.17", hasYen: true),
            InvoiceParser.AmountCandidate(x: 200, amount: "11.83", hasYen: true),
        ]
        let (before, tax) = InvoiceParser.pickAmountPair(cands, totalWithTax: "209.00")
    ...
    func testPickAmountPair_prefersYenOverBareContaminant() {
        ...
        let cands = [
            InvoiceParser.AmountCandidate(x: 57, amount: "11.83", hasYen: false),
            InvoiceParser.AmountCandidate(x: 196, amount: "197.17", hasYen: true),
            InvoiceParser.AmountCandidate(x: 412, amount: "11.83", hasYen: true),
        ]
        let (before, tax) = InvoiceParser.pickAmountPair(cands, totalWithTax: "209.00")
        XCTAssertEqual(before, "197.17")
        XCTAssertEqual(tax, "11.83")
    ...
    func testPickAmountPair_fallbackRightmost() {
        let cands = [
            InvoiceParser.AmountCandidate(x: 10, amount: "1.00", hasYen: true),
            InvoiceParser.AmountCandidate(x: 20, amount: "2.00", hasYen: true),
            InvoiceParser.AmountCandidate(x: 30, amount: "3.00", hasYen: true),
        ]
        let (before, tax) = InvoiceParser.pickAmountPair(cands, totalWithTax: "999.00")
    ...
    func testPickAmountPair_zeroTax() {
        let cands = [
            InvoiceParser.AmountCandidate(x: 100, amount: "2000.00", hasYen: true),
            InvoiceParser.AmountCandidate(x: 200, amount: "0.00", hasYen: true),
        ]
        let (before, tax) = InvoiceParser.pickAmountPair(cands, totalWithTax: "2000.00")
    ...
    func testYenAmounts_twoAmounts() {
        let line = InvoiceTokenizer.Line(y: 0, tokens: [
            InvoiceTokenizer.Token(x: 10, y: 0, s: "¥2000.00"),
            InvoiceTokenizer.Token(x: 20, y: 0, s: "¥0.00"),
        ])
        XCTAssertEqual(InvoiceParser.yenAmounts(in: line), ["2000.00", "0.00"])
    }

    func testYenAmounts_empty() {
        let line = InvoiceTokenizer.Line(y: 0, tokens: [
            InvoiceTokenizer.Token(x: 10, y: 0, s: "464.15"),  // bare, no ¥
        ])
        XCTAssertTrue(InvoiceParser.yenAmounts(in: line).isEmpty)
    }

    func testAmountCandidates_mixedYenAndBare() {
        let line = InvoiceTokenizer.Line(y: 0, tokens: [
            InvoiceTokenizer.Token(x: 56, y: 0, s: "2000.00"),    // bare
            InvoiceTokenizer.Token(x: 180, y: 0, s: "¥2000.00"),  // yen
            InvoiceTokenizer.Token(x: 469, y: 0, s: "¥0.00"),     // yen
        ])
        let cands = InvoiceParser.amountCandidates(in: line)
    ...
    func testAmountCandidates_excludesTaxRates() {
        let line = InvoiceTokenizer.Line(y: 0, tokens: [
            InvoiceTokenizer.Token(x: 10, y: 0, s: "6%"),
            InvoiceTokenizer.Token(x: 20, y: 0, s: "197.17"),
        ])
        let cands = InvoiceParser.amountCandidates(in: line)
    ...
    func testGroupIntoLines_byY() {
        ...
        let tokens = [
            InvoiceTokenizer.Token(x: 10, y: 100, s: "A"),
            InvoiceTokenizer.Token(x: 20, y: 100, s: "B"),
            InvoiceTokenizer.Token(x: 10, y: 50, s: "C"),
        ]
        let lines = InvoiceTokenizer.groupIntoLines(tokens, tolerance: 3)
    ...
    func testGroupIntoLines_withinTolerance() {
        ...
        let tokens = [
            InvoiceTokenizer.Token(x: 10, y: 100, s: "A"),
            InvoiceTokenizer.Token(x: 20, y: 102, s: "B"),
        ]
        let lines = InvoiceTokenizer.groupIntoLines(tokens, tolerance: 3)
    ...
    func testParse_mixedTotalAndGrandTotalLine() {
        let lines = [
            InvoiceTokenizer.Line(y: 100, tokens: [
                .init(x: 10, y: 0, s: "不征税"),
                .init(x: 50, y: 0, s: "合"),
                .init(x: 70, y: 0, s: "计"),
            ...
            InvoiceTokenizer.Line(y: 90, tokens: [
